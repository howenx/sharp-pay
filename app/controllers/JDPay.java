...
public class JDPay extends Controller {
        ...
        try {
            ...
            if (listOptional.isPresent() && listOptional.get().size() > 0) {
                ...
                if (longOptional.isPresent() && longOptional.get() < 0) {
                    ...
                    Map<String,Boolean> payShowMap=new HashMap<>();
                    payShowMap.put("JD",SysParCom.PAY_SHOW_JD);
                    payShowMap.put("WEIXIN",SysParCom.PAY_SHOW_WEIXIN);
    ...
    private Map<String, String> getOrderInfo(Long userId, Long orderId) throws Exception {
        ...
        if (listOptional.isPresent() && listOptional.get().size() > 0) {
            ...
            map.put("trade_subject", SysParCom.PAY_ORDER_DESC + orderId);
            if (ONE_CENT_PAY) {
